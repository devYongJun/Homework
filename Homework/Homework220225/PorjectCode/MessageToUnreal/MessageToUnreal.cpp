...
#include <vector> // 숙제코드 추가

#pragma comment(lib, "GameServerBase.lib")

// 숙제코드 추가
// 파일을 Write하는 작업을 전부 writeWorks 백터에 담아서 나중에 한꺼번에 처리
std::vector<std::function<void()>> writeWorks;
...
void MessageHeaderCreate(std::vector<MessageInfo>& _Collection, const std::string Path)
{
	...
	for (size_t i = 0; i < _Collection.size(); i++)
	{
		...
		{
			// 숙제코드 추가
			serializeResult = SerializerTypeCheck(MessageText, MemberList[m]);
			if (!serializeResult)
				break;
		}
		MessageText += "\n";
		...
		{
			// 숙제코드 추가
			deserializeResult = DeSerializerTypeCheck(MessageText, MemberList[m]);
	...
	// 숙제코드 추가
	if (serializeResult && deserializeResult)
	{
		writeWorks.push_back([Path, MessageText]() {
...
int main()
{
	...
	/////////////////////////////////// 서버 파일생성
	{

		{
			...
			// 숙제코드 추가
			writeWorks.push_back([SavePath, EnumFileText]() {
...

		{
			...
			// 숙제코드 추가
			writeWorks.push_back([SavePath, ConvertFileText]() {
				GameServerFile SaveFile = GameServerFile{ SavePath, "wt" };
