...
void ULoginUI::JoinUser()
{
	...
	if (Inst->ID == IDString)
	{
		UE_LOG(ClientLog, Error, TEXT("%S(%u) > DUPLICATE"), __FUNCTION__, __LINE__);
		return;
	}
	...
	{
		Inst->ID = IDString;
		UE_LOG(ClientLog, Error, TEXT("%S(%u) > SEND OK"), __FUNCTION__, __LINE__);
	}
	else
	{
		UE_LOG(ClientLog, Error, TEXT("%S(%u) > SEND FAIL"), __FUNCTION__, __LINE__);
	}
}