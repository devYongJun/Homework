...
protected:
	...
	////////////////// 액터관리
protected:
	virtual void SectionUpdate(float _Delta) = 0;
	...
	// 1
	std::map<uint64_t, std::shared_ptr<GameServerActor>> KeyActors_;

	std::list<std::shared_ptr<GameServerActor>> AIActors_;
	std::list<std::shared_ptr<GameServerActor>> PlayableActors_;

	std::atomic<size_t> WaitJoinActorCount_;
	std::mutex WaitJoinLock;
	std::vector<std::shared_ptr<GameServerActor>> WaitJoinActors_;
