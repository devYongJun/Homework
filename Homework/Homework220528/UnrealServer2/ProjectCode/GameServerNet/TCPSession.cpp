...
TCPSession::TCPSession() 
	...
	RecvMode_(RecvMode::Start)
{
	AppendBuffer = std::vector<unsigned char>();
	AppendBuffer.resize(1024);
}
...
void TCPSession::OnRecv(const char* _Data, DWORD _ByteSize)
{
	...
	if (RecvMode_ == RecvMode::Start)
	{
		...
		RecvSize = 0;
		RecvOffset = 0;
		memcpy_s(&RecvSize, 4, &_Data[4], 4);

		if (AppendBuffer.size() < RecvSize)
		{
			AppendBuffer.resize(RecvSize);
		}
		else if(AppendBuffer.size() > 1024)
		{
			AppendBuffer = std::vector<unsigned char>();
			AppendBuffer.resize(1024);
		}
	...
	memcpy_s(&AppendBuffer[RecvOffset], _ByteSize, _Data, _ByteSize);

	// 1024
	RecvOffset += _ByteSize;

	if (RecvOffset != RecvSize)
	{
		RecvMode_ = RecvMode::Wait;
		// 아직 더 읽을게 있든
		// 크기가 작게 되서 에러가 났든.
		RecvRequest();
		return;
	...
	{
		RecvCallBack_(std::dynamic_pointer_cast<TCPSession>(shared_from_this()), AppendBuffer);
	}
