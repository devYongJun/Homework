...
public:
	RECVOVERDATA() 
	{
		...
		Buf.len = 1024;
		Buf.buf = reinterpret_cast<char*>(ArrBuffer);
	}
};
...
void IocpThread(size_t _Index)
{
	std::wstring Str = L"Thread";
	Str += std::to_wstring(_Index);
	SetThreadDescription(GetCurrentThread(), Str.c_str());
...
int main()
{
	...
	{
		threadList_.push_back(std::make_shared<std::thread>(std::bind(IocpThread, i)));
	}
